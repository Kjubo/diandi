...
- (void)storeValue:(NSString *)value dir:(NSString *)dir key:(NSString *)key{
    [self storeValue:value dir:dir key:key expire:[NSDate dateWithTimeIntervalSinceNow:D_YEAR]];
}
...
- (void)removeDir:(NSString *)dir{
    NSString *newDir = [dir trim];
    //从数据库删除
    [_db executeUpdate:@"delete from cachetb where cdir=?", newDir];
    //从二级缓存中移除
    [_valuesCached removeAllObjects];
    [_valuesKeyList removeAllObjects];
}
...
//从二级缓存中移出数据
- (void)removeCacheFieldsFromValueDic:(NSString *)dicKey{
    if ([dicKey length] > 0) {
        id x = [_valuesCached valueForKey:dicKey];
        if (x) {
            [_valuesCached removeObjectForKey:dicKey];
        }
    }
}
