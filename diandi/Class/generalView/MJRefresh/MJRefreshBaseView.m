...
{
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 32, 32)]; //[[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"arrow.png")]];
...
{
    if (!_activityView) {
        UIImageView *activityView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"lodingring"]];
        activityView.center = self.arrowImage.center;
        activityView.autoresizingMask = self.arrowImage.autoresizingMask;
        [self addSubview:_activityView = activityView];
    }
    if(![_activityView.layer animationForKey:@"rotationAnimation"]){
        ...
        [_activityView.layer addAnimation:rotaionAnimation forKey:@"rotationAnimation"];
    }
...
{
    if (!_logoView) {
        UIImageView *logoView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"lin"]];
        logoView.center = self.arrowImage.center;
        logoView.autoresizingMask = self.arrowImage.autoresizingMask;
...
- (void)layoutSubviews
{
    ...
    // 2.0 中心logo
    self.logoView.center = self.arrowImage.center;
    // 2.指示器
