...
- (void)layoutSubviews
{
    ...
    // 1.状态标签
    self.statusLabel.size = CGSizeMake(self.mj_width, 20);
    self.statusLabel.bottom = self.mj_height/2;
    // 2.时间标签
    self.lastUpdateTimeLabel.size = self.statusLabel.size;
    self.lastUpdateTimeLabel.top = self.statusLabel.bottom;
}
...
- (void)drawRect:(CGRect)rect{
    ...
    CGContextSetStrokeColorWithColor(context, GS_COLOR_MAIN.CGColor);
    CGFloat startAngle = 0;
    
    CGFloat scale = (fabs(self.scrollView.contentOffset.y) - self.activityView.top)/self.height;
    CGFloat angle = MIN(scale, 1.0) * 2 * M_PI ;
    //设置要画的形状
    CGContextAddArc(context, self.arrowImage.centerX, self.arrowImage.centerY, 13, startAngle, startAngle+angle, 0);
    // 第三步
